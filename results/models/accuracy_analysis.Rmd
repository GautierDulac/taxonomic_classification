...
colnames(NB_results) <- c('HVR', 'TAXO_rank', 'test_size', 'main_class_prop', 'NB_4', 'NB_3', 'NB_2', 'NB_1')
```
...
NB_gathered_results <- NB_results %>% 
  gather('NB_k', 'Accuracy', NB_4, NB_3, NB_2, NB_1) %>% 
...
## Ploting the comparison of different NB(k) results for a given HVR (V4) 

```{r}
meth <- 'Naive_Bayes_up_to_4'
```

```{r}
ONE_HVR <- c('V1', 'V2', 'V3', 'V4', 'V5', 'V6', 'V7', 'V8', 'V9')
...
```{r}
NB_gathered_results <- NB_results %>% 
  gather('NB_k', 'Accuracy', NB_4, NB_3, NB_2, NB_1, main_class_prop) %>% 
