...
class MyTestCase(unittest.TestCase):
    def test_loading_sequences(self):
        ...
        df = pd.read_csv(project_directory + '\\data\\complete_gg_13_5_otus_rep_set_complete.csv'.format(
            int(sampling * 100)))
        ...
        sampling = 0.01
        main_loading(sampling=sampling)
        df = pd.read_csv(project_directory + '\\data\\complete_gg_13_5_otus_rep_set_sampled_{}_percent.csv'.format(
            int(sampling * 100)))
        target_columns = ['reference', 'sequence', 'sequence_size', 'file_num']
        self.assertEqual(len(df.columns), len(target_columns))
        for col_id in range(len(target_columns)):
